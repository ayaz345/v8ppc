...
class TestSuiteTest(unittest.TestCase):
  def test_filter_testcases_by_status_first_pass(self):
    ...
    suite.rules = {'': {'foo/bar': {'PASS', 'SKIP'}, 'baz/bar': {'PASS', 'FAIL'}}}
    suite.wildcards = {'': {'baz/*': {'PASS', 'SLOW'}}}
    suite.FilterTestCasesByStatus(warn_unused_rules=False)
    self.assertEquals(
        [TestCase(suite, 'baz/bar')],
        suite.tests,
    )
    self.assertEquals({'PASS', 'FAIL', 'SLOW'}, suite.tests[0].outcomes)
  ...
  def test_filter_testcases_by_status_second_pass(self):
    ...
    self.assertEquals({'PASS', 'SLOW', 'PREV'}, suite.tests[0].outcomes)
    self.assertEquals({'PASS', 'FAIL', 'SLOW', 'PREV'}, suite.tests[1].outcomes)
